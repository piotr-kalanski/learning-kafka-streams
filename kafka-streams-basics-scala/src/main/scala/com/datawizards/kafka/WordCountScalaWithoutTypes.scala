...
object WordCountScalaWithoutTypes extends App {
  ...
  val textLines = builder.stream("word-count-input")

  val wordCounts = textLines
    // 2 - map values to lowercase
    .mapValues[String]((textLine: String) => textLine.toLowerCase)
    // 3 - flatmap values split by space
    .flatMapValues[String]((textLine: String) => textLine.split("\\W+").toIterable.asJava)
